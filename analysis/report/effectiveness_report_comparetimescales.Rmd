...
print_plots_diff <- function(outcome, censor_seconddose){
  ...
  plot_diff <-
    ggplot(curves)+
    geom_hline(aes(yintercept=0, colour=model_name))+
    geom_step(aes(x=tstop, y=diff, colour=model_name))+
    geom_rect(aes(xmin=lag(tstop, 1, 0), xmax=tstop, ymin=diff.ll, ymax=diff.ul, fill=model_name), alpha=0.1, colour="transparent")+
    facet_grid(cols=vars(timescale))+
    scale_x_continuous(
      limits = c(0, NA),
      breaks = seq(0,7*52,by=14),
