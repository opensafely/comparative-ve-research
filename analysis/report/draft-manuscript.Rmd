...
```{r, effects, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  riskdiff <-
    map_dfr(
      outcomes,
      ~read_rds(here("output", "models", .x, "timesincevax", glue("reportplr_adjusteddiff_ns.rds"))),
      .id="outcome"
    ) %>%
    left_join(metadata_outcomes, by="outcome") %>%
    mutate(
...
At 6 weeks post-vaccination, the absolute risk difference (ChAdOx1 - BNT162b2) for a positive SARS-CoV-2 test was `r filter(riskdiff, outcome=="postest", tstop==7*6)$diff_format` per 1000 people, for A&E attendances was `r filter(riskdiff, outcome=="emergency", tstop==7*6)$diff_format` and for COVID-19 hospital admissions was `r filter(riskdiff, outcome=="covidadmitted", tstop==7*6)$diff_format` per 1000 people.
