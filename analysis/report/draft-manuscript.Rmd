...
A total of `r flowchart[1, "n"]` HCWs aged 18-64 receiving a first dose of BNT162b2 or ChAdOx1 between 4 January and 28 February 2021 and actively registered at at TPP practice were identified, with `r flowchart[[nrow(flowchart), "n"]]` (`r label_percent(0.1)(flowchart[[nrow(flowchart), "pct_all"]])`) meeting the study eligibility criteria. `r baseline_pfizer$n` (`r label_percent(0.1)(baseline_pfizer$n/baseline$n)`) were vaccinated with BNT162b2, contributing `r label_number(0.1)(baseline_pfizer$fu_years)` person-years of follow-up. For ChAdOx1, this was `r baseline_az$n` (`r label_percent(0.1)(baseline_az$n/baseline$n)`) and `r label_number(0.1)(baseline_az$fu_years)` person-years.
...
Table 2 shows event rates by vaccine type, and the crude incidence. Over the duration of `r label_number(0.1)(baseline$fu_years)` person-years of follow-up there were `r baseline$postest` positive SARS-CoV-2 tests, `r baseline$emergency` A&E attendances, `r baseline$covidadmitted` COVID-19 hospital admissions, and `r baseline$coviddeath` COVID-19 deaths.
...
```{r, effects, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  riskdifference <- 
    ...
    ) #-(surv-lag(surv))/lag(surv)
    
  
  plot_riskdiff <-
    ggplot(riskdifference)+
...
At 6 weeks post-vaccination, the absolute risk difference (ChAdOx1 - BNT162b2) for a positive SARS-CoV-2 test was `r filter(riskdifference, outcome=="postest", tstop==7*6)$diff_format` per 1000 people, for A&E attendances was `r filter(riskdifference, outcome=="emergency", tstop==7*6)$diff_format` and for COVID-19 hospital admissions was `r filter(riskdifference, outcome=="covidadmitted", tstop==7*6)$diff_format` per 1000 people.

#### Figure 2: Comparative effectivevess

```{r comp,echo=FALSE, message=FALSE, out.width='90%', fig.height=8, results='asis'}
