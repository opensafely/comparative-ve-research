...
print_plots <- function(outcome, outcome_descr){
  ...
  calendar_image <- here("output", outcome, "calendar", glue("effect_plot.png"))
  timesincevax_image <- here("output", outcome, "timesincevax", glue("effect_plot.png"))
  ...
  calendar_effects <- read_rds(here("output", outcome, "calendar", glue("effect_estimates.rds"))) %>%
    mutate(timescale = "Calendar time")
  timesincevax_effects <- read_rds(here("output", outcome, "timesincevax", glue("effect_estimates.rds"))) %>%
    mutate(timescale = "Vaccinated time")
  
  effects <- bind_rows(calendar_effects, timesincevax_effects)
  ...
  effects_plot <-
    ...
    scale_x_continuous(breaks=unique(effects$term_left), limits=c(min(effects$term_left), max(effects$term_right)+1), expand = c(0, 0))+
    scale_y_log10(
  ...
  # 
  # calendar_plot <- read_rds(here("output", outcome, "calendar", glue("effect_plot.rds"))) + 
  #   labs(title=NULL)+
  #   coord_cartesian(ylim=c(0.25,4))+
  #   theme(text = element_text(size=10))
  # 
  # timesincevax_plot <- read_rds(here("output", outcome, "timesincevax", glue("effect_plot.rds"))) + 
  #   labs(title=NULL)+
  #   coord_cartesian(ylim=c(0.25,4))+
  #   theme(text = element_text(size=10))
  # 
  ...
  cat("  \n\n")
  print(glue("### {outcome_descr}"), "  \n")
  print(km_plot)
  print(effects_plot)
  cat("  \n\n")
  # wrap_plots(
  #  km_plot + labs(title=outcome_descr),
  #  effects_plot,
  #  ncol=1
  # ) +
