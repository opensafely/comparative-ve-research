...
fs::dir_create(output_dir, "report", "figures-vaxtime")

knitr::opts_chunk$set(
  echo = TRUE,
  fig.path = paste0(fs::path(output_dir, "report", "figures-vaxtime"), "/")
)
...
Time zero is the date of vaccination. Calendar-time is included as an additional covariate using restricted cubic spline with knots at the first and second tertile, plus boundary knots.
...
```{r, curves_vaxtime, echo=FALSE, message=FALSE, warning=FALSE, out.width='90%', results='asis'}
...
print_plots_effects <- function(outcome, modeltype){
  ...
  tidy_effects <- read_rds(here("output", "models", outcome, "timesincevax", glue("report{modeltype}_effects{suffix}.rds"))) 
...
print_plots_cmlinc <- function(outcome, spline){
  ...
  curves <- read_rds(here("output", "models", outcome, "timesincevax", glue("reportplr_cmlinccurves_{spline}.rds"))) 
